...
import { TodoEntity, TodoLabelEntity, todoLabelModel, todoModel } from '../../model/todos'
import { CHANNEL } from '../../model/channel'
import { CustomerEntity, CustomerLabelEntity, customerModel, labelModel } from '../../model/customer'
...
export const createScrumboard = async (
  ...
  next: NextFunction) => {
    ...
    try {
      ...
      newBoard.settings = {color:board.settings.color, subscribed:board.settings.subscribed, cardCoverImages:board.settings.cardCoverImages}
...
export const renameScrumboard =async (
  ...
  next: NextFunction) => {
    ...
    try {      
      ...
      let boardItem = await boardModel.getBoardWithId(boardId, organization) as BoardEntity
      if (!boardItem) {
        errorMessage('CONTROLLER', 'rename board', 'invalid boardId')
        return next(new HttpException(400, ErrorCode[400]))
      }

      const updateResult = await boardModel.updateBoardName(boardId, boardTitle, requester)
      if (!updateResult) {
...
export const renameListTitle =async (
  ...
  next: NextFunction) => {
    ...
    try {      
      ...
      let listItem = await listModel.getlistWithId(listId, organization) as ListEntity
      if (!listItem) {
        errorMessage('CONTROLLER', 'rename List Title', 'invalid listId')
        return next(new HttpException(400, ErrorCode[400]))
      }

      const updateResult = await listModel.updateListName(listId, listTitle, chatType, chatLabels)
      if (!updateResult) {
        errorMessage('CONTROLLER', 'update board', 'board not update')
        return next(new HttpException(404, 'board not update'))
      }
...
export const getScrumboardByID = async (
  ...
  next:NextFunction) => {    
    ...
    if (!boardID || typeof boardID !== 'string') {
        errorMessage('CONTROLLER', 'getScrumboardByID', 'invalid parameter(boardId)')
        return next(new HttpException(400, ErrorCode[400]))
    }
    const organization: OrganizationEntity = req.body.organization
    const requester: UserEntity = req.body.requester

    const board = await getBoardData(boardID, organization, requester, pageNumber, chagenListId)
...
export const getBoardData = async (boardID:string, organization:OrganizationEntity, requester: UserEntity, pageNumber:number=0, chagenListId:string='') => {

    const board = await boardModel.getBoardWithId(boardID, organization) as BoardEntity
    ...
    for (let i = 0; i < board.lists.length; i ++){
      ...
      const oldCards = await cardModel.getChatCardsWithList(board.lists[i], organization)
      ...
      for(let j = 0; j < chats.length; j ++) {
        let tempCard = await cardModel.getCardWithChatId(chats[j].id, board.lists[i], organization)
        if (tempCard) {
        ...
        if (!tempCard){
          ...
          cardItem.orderIndex = await cardModel.getMaxOrderIndex(board.lists[i],organization) + 1        
          await cardModel.saveCard(cardItem)
        }
      }

      let listCards = await cardModel.getCardsWithListId(board.lists[i], organization) as CardEntity[]
      ...
      for (let j = 0; j < board.lists[i].idCards.length; j++){
        ...
        let cardData = await getCardData(cardItem.id, organization) as CardEntity    
        if (cardData) {  
      ...
      let _listCards = board.lists[i].idCards.slice(0, _pageNumber * CARD_COUNT + CARD_COUNT)
    ...
    board.labels = await todoLabelModel.getLabels(organization) as TodoLabelEntity[]

    for (let i = 0; i < board.lists.length; i ++) {
      for (let j = 0; j < board.lists[i].idCards.length; j++) {
        if (board.lists[i].idCards[j].chatId.length > 0){
          let _chat = await getChat(board.lists[i].idCards[j].chatId, organization, requester) as ChatEntity
...
export const getCardData = async (cardID:string, organization:OrganizationEntity) => {
  
  let cardData = await cardModel.getCardWithId(cardID, organization) as CardEntity
  if (!cardData) {
    return null
  }
...
export const createScrumboardList = async (
  ...
  next:NextFunction) => {
    ...
    try {      
      ...
      listItem.orderIndex = await listModel.getMaxOrderIndex(organization, boardItem) + 1
      ...
      for (let i = 0; i < chats.length; i++) {
        ...
        cardItem.organization = organization
        cardItem.createdBy = requester
        cardItem.orderIndex = await cardModel.getMaxOrderIndex(saveResult,organization) + 1        
        await cardModel.saveCard(cardItem)
      }
...
export const getChat = async (
  ...
  requester: UserEntity) => {
    ...
    let chats = await chatModel.getChatsWithId(chatId, organization) as ChatEntity[]
...
export const createCardComment = async (
  ...
  commentItem.card = await cardModel.getCardWithIdUnRelation(cardId, organization) as CardEntity
...
export const updateScrumboardCard = async (
  ...
  try {
    ...
    let cardListData = await listModel.getlistWithId(card.listId, organization) as ListEntity
    let cardBoardData = await boardModel.getBoardWithId(card.boardId, organization) as BoardEntity
    ...
    if (idLabels) {
      ...
      for(let i = 0; i < idLabels.length; i ++) {
        ...
        let todoLbl = await todoLabelModel.getLabelWithId(idLabels[i], organization) as TodoLabelEntity
        newTodoLbls.push(todoLbl)
      }
    }
    
    const oldMembers = await cardMemberModel.getCardMembers(card.id) as CardMemberEntity[]
    for(let i = 0; i < oldMembers.length; i ++) {
      if (!idMembers.find((element: string) => element == oldMembers[i].memberId)){
        let oldTodo = await todoModel.getTodoWithCardId(card.id, oldMembers[i].memberId, organization) as TodoEntity
        if (oldTodo) {
    ...
    const cardMembers = await cardMemberModel.getCardMembers(card.id) as CardMemberEntity[] 
    for(let i = 0; i < cardMembers.length; i++) {
      let todoItem = await todoModel.getTodoWithCardId(card.id, cardMembers[i].memberId, organization) as TodoEntity
      if (todoItem) {
        // todoItem.title = udpateCardData.name ? udpateCardData.name : todoItem.title
        todoItem.title = cardBoardData.name + ' > ' + cardListData.name
        todoItem.notes = udpateCardData.description ? udpateCardData.description : todoItem.notes
        ...
        newTodo.title = cardBoardData.name + ' > ' + cardListData.name
        newTodo.notes = udpateCardData.description ? udpateCardData.description : ''
        if (udpateCardData.due > 0) {
...
export const removeList =async (
  ...
  next:NextFunction) => {  
    ...
    let oldData = await listModel.getlistWithId(listId, organization) as ListEntity
    if (!oldData) {      
    ...
    for (let i = 0; i < oldData.idCards.length; i ++) {
      let _card = await getCardData(oldData.idCards[i].id, organization) as CardEntity
      if (!_card) {
        _card = await cardModel.getCardWithDelete(oldData.idCards[i].id, organization) as CardEntity
      }
      if (_card) {
        ...
        if (_card.checklists){
          for (let i = 0; i < _card.checklists.length; i ++) {
            await checklistItemModel.deleteChecklistItemWithChecklist(_card.checklists[i])
  ...
  try {
    ...
    if (cardData.checklists){
      for (let i = 0; i < cardData.checklists.length; i ++) {
        await checklistItemModel.deleteChecklistItemWithChecklist(cardData.checklists[i])
...
export const createCardChecklist = async (
  ...
  try {
    ...
    checklist.card = await cardModel.getCardWithIdUnRelation(cardId, organization) as CardEntity
    ...
    if (saveResult){
      const checklist = await checklistModel.getChecklistWithChatId(saveResult.id, cardId, organization)
      return res.status(200).send(checklist)
    }else{
      errorMessage('CONTROLLER', 'create card checklist', 'checklist not create')
...
export const createChecklistItem = async (
  ...
  try {
    ...
    checklistItem.checklist = await checklistModel.getChecklistWithId(checklistId, organization) as ChecklistEntity
    ...
    if (saveResult){
      ...
      errorMessage('CONTROLLER', 'create card checklistItem', 'checklistItem not create')
      return next(new HttpException(404, 'checklistItem not create'))
    }
  } catch (error) {
    errorMessage('CONTROLLER', 'create check list item', 'create check list item', error)
    return next(new HttpException(500, ErrorCode[500]))
  }
}
...
export const changelistItemStatus = async (
  ...
  try {
    ...
    if (!itemId) {
      errorMessage('CONTROLLER', 'change status of checklistItem', 'invalid data')
      return next(new HttpException(400, ErrorCode[400]))
    }

    let checklistItem = await checklistItemModel.getChecklistItemWithId(itemId, organization) as ChecklistItemEntity
    ...
    if (saveResult){
      ...
      errorMessage('CONTROLLER', 'change status of checklistItem', 'status  not change')
      return next(new HttpException(404, 'checklistItem not create'))
    }
  } catch (error) {
    errorMessage('CONTROLLER', 'change status of checklistItem', 'status  not change', error)
    return next(new HttpException(500, ErrorCode[500]))
  }
}
...
export const deleteChecklistItem = async (
  ...
  try {
    ...
    errorMessage('CONTROLLER', 'delete checklistItem', 'delete checklistItem', error)
    return next(new HttpException(500, ErrorCode[500]))
  }
}
...
export const updateChecklistItem = async (
  ...
  try {
    ...
    errorMessage('CONTROLLER', 'update checklistItem', 'update checklistItem', error)
    return next(new HttpException(500, ErrorCode[500]))
  }
}
...
export const updateChecklist = async (
  ...
  try {
    ...
    checklist.name = listName
    checklist.card = await cardModel.getCardWithIdUnRelation(_checklist.cardId, organization) as CardEntity
    checklist.organization = organization
...
export const removeChecklist = async (
  ...
  try {
    ...
    const checklist = await checklistModel.getChecklistWithId(checklistId, organization) as ChecklistEntity
...
export const updateBoardSetting = async (
  ...
  try {    
    ...
    let boardData = await boardModel.getBoardWithId(boardId, organization) as BoardEntity
    if (!boardData) {
      errorMessage('CONSTROLLER', 'update board setting', 'board not found')
      return next(new HttpException(400, ErrorCode[400]))
    }

    let updateResult = await boardModel.updateBoardSetting(boardId, settings, requester)
    if(updateResult) {
    ...
    errorMessage('CONTROLLER', 'update board setting', 'update borad setting', error)
    return next(new HttpException(500, ErrorCode[500]))
  }
}
...
export const sendFileAttachment = async (
  ...
  try {
    ...
    attachment.card = await cardModel.getCardWithIdUnRelation(cardId, organization) as CardEntity
    attachment.time = Math.floor((new Date()).getTime() / 1000);

    const saveResult = await attachmentModel.saveAttachment(attachment)
    if (saveResult){
      const cardData = await cardModel.getCardWithIdUnRelation(cardId, organization) as CardEntity
      const attachments = await attachmentModel.getAttachmentWithCard(cardData, organization)
...
export const removeAttachmentFile = async (
  ...
  try {
    ...
    const oldData = await attachmentModel.getAttachmentWithId(item.id, organization) as CardAttachmentEntity
...
export const cardOrderInit =async (
  ...
  for (let i = 0; i < lists.length; i++){
    ...
    for (let j = 0; j < lists[i].idCards.length; j++){
      const listItem = await listModel.getlistWithId(lists[i].id, organization) as ListEntity
      if (sourceCardId == lists[i].idCards[j].id){
        let cardData = await cardModel.getCardWithId(sourceCardId, organization)
        if (cardData && cardData.chatId.length > 0 && lists[i].id != cardData.list.id) {
          let chatType = lists[i].chatType
          let chatLabels = lists[i].chatLabels
          let chatData = await chatModel.getChatWithId(cardData.chatId, organization) as ChatEntity
          ...
          if (chatLabels.length > 0) {
            ...
            const currentLabels = customer.customerLabel as CustomerLabelEntity[]
            const sourceList = await listModel.getlistWithId(sourceListId, organization) as ListEntity
            const sourceLabelsStr = sourceList.chatLabels
            const sourceLabels = sourceLabelsStr.split(',')
            for (let k = 0; k < sourceLabels.length; k ++){
              let oldIndex = currentLabels.findIndex((e) => e.label == sourceLabels[k])
              if (oldIndex > -1) {
            ...
            for (let k = 0; k < inputLabel.length; k ++) {
              let customerLbl = await labelModel.getLabelWithName(inputLabel[k], organization) as CustomerLabelEntity
              if (customerLbl) {
...
export const getChatMessage = async (
  ...
  try {
    ...
    let messages = convertMessage.slice(0, pageNumber * MESSAGE_COUNT+MESSAGE_COUNT)
