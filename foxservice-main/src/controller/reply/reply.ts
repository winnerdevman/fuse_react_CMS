...
import { ReplyEntity, ReplyKeywordEntity, replyModel, ReplyResponseEntity } from '../../model/reply'
...
export const deleteReply = async (
  ...
  try {
    const replyResult = await replyModel.getReplyWithId(id, organization)
    if (!replyResult) {
      errorMessage('CONTROLLER', 'reply', ' Reply not found')
      return next(new HttpException(404, 'reply not found'))
    }
